...
Use a SINGLE `AskUserQuestion` call with 2 questions (single repo) or 3 questions (monorepo).
The tool supports up to 4 questions per call, each with up to 4 options. All multiSelect: true.
...
Question 3 (MONOREPO only):
- question: "Monorepo-specific docs to skip?"
- header: "Skip mono?"
- multiSelect: true
- options:
